...
const AutoComplete = (): ReactElement => {
  ...
  const handleSelectingSuggestion = (name: string) => {
    setSearchQuery(name);
  }
    ...
    <div className='flex justify-center mt-20'>
      <div className='w-[40vw] h-[3vw] border-1 border-gray-950'>
        <div className='flex h-full'>
          <input className='w-[90%] h-full focus:border-transparent focus:outline-none py-1.5 px-2'
            value={searchQuery}
          ...
          <button className='w-[10%] h-full bg-blue-600 text-white cursor-pointer'
          ...
          <div className='w-full rounded-b-2xl border-1 border-l-black border-r-black border-b-black'>
            {
              suggestions.map((suggestion, index) => (
                <div key={suggestion.id} className={`flex w-full h-[2vw] px-2 py-1 ${(index < suggestions.length - 1) ? 'border-b-gray-500 border-b' : ''} cursor-pointer hover:bg-blue-400`}
                  onMouseDown={() => handleSelectingSuggestion(suggestion.name)}>
                  {suggestion.name}
                </div>
              ))
            }
